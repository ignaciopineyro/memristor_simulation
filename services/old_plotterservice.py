...
for variable_param in list(pershin_params):
    ...
    for file_count, filepath in enumerate(csv_files_path_in_directory, 1):
        df = pd.DataFrame(
            np.concatenate(
                [
                    pd.read_csv(filepath, sep=r"\s+")[
                        1 : len(pd.read_csv(filepath, sep=r"\s+")) + 1
                    ]
                ]
            ),
        ...
        plt.plot(
            df["vin"],
            -df["i(v1)"],
            label=f"{variable_param} = {pershin_values[variable_param][file_count-1]}",
        )
        plt.xlabel("Vin [V]")
        plt.ylabel("i(v1) [A]")
        plt.title(
            f"I-V {model} - {variable_param} = {pershin_values[variable_param]}",
            fontsize=18,
        )
        plt.legend()
        ...
        plt.ylabel("i(v1) [A]")
        plt.title(
            f"{variable_param} = {pershin_values[variable_param][file_count - 1]}",
            fontsize=18,
        )
        ...
        plt.ylabel("i(v1) [A]")
        plt.title(
            f"I-V {model} - {variable_param} = {pershin_values[variable_param][file_count - 1]}",
            fontsize=16,
        )
        plt.savefig(
            f"{path}/{simulations_path}/{csv_files_in_directory[file_count - 1]}.jpg"
        )
        ...
        plt.suptitle(
            f"Input voltage and State vs Time {model} {pershin_params}", fontsize=16
        )
        plt.legend()
        plt.savefig(
            f"{path}/{simulations_path}/{csv_files_in_directory[file_count - 1]}_states.jpg"
        )
