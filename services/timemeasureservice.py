...
from representations import TimeMeasure, AverageTimeMeasure
...
class TimeMeasureService:
    ...
    @staticmethod
    def compute_time_average(time_measures: List[TimeMeasure], amount_iterations: int) -> AverageTimeMeasure:
        average_python_execution_time = (
                sum(time_measure.python_execution_time for time_measure in time_measures) / amount_iterations
        )
        average_linux_real_execution_time = (
                sum(time_measure.linux_real_execution_time for time_measure in time_measures) / amount_iterations
        )
        average_linux_user_execution_time = (
                sum(time_measure.linux_user_execution_time for time_measure in time_measures) / amount_iterations
        )
        average_linux_sys_execution_time = (
                sum(time_measure.linux_sys_execution_time for time_measure in time_measures) / amount_iterations
        )

        return AverageTimeMeasure(
            amount_iterations, average_python_execution_time, average_linux_real_execution_time,
            average_linux_user_execution_time, average_linux_sys_execution_time
        )

    def write_python_time_measure_into_csv(self, time_measure: TimeMeasure) -> TimeMeasure:
